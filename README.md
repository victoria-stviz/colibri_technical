...
Layers
Bronze: 
  Insert of raw data with a 24-hour cadence. There’s a bronze table for each file received.
...
Silver: 
  Delta load of records from the bronze tables. The reference column for the delta load is the record_load_timestamp. As the records don’t change once they’re created, there’s no merging of records.
...
Gold layer: 
  Two tables make up the Gold layer:
  ...
  For the summary data, a view is selected as it's assumed history doesn't need to be persisted and it's going to be used as a status on any one day. Using a view instead of a managed table reduces the storage used.

Issues I ran into: some of the things like creating delta tables or storing into another database couldn't be done as I couldn't complete the JAVA setup in my laptop. There are other things like unit testing and schema assessment that I would've liked to do using a Databricks environment where I feel more comfortable working on.
